...
public class CommunicationThread {
	...
	public CommunicationThread(final ServerSocket socket, final SideEffect1<SafeDataOutputStream> sendInitialData,
	...
	private FutureTask<ExitCode> createFuture() {
		final FutureTask<ExitCode> newFuture = new FutureTask<ExitCode>(
				new SocketReadingCallable(this.socket, this.sendInitialData, this.receive));
	...
	public ExitCode waitToFinish() {
		try {
			...
			throw new PitError("Error while watching child process", e);
			//return ExitCode.UNKNOWN_ERROR;
		} catch (final InterruptedException e) {
			LOG.log(Level.WARNING, "interrupted while waiting for child process", e);
			throw new PitError("interrupted while waiting for child process", e);
			//return ExitCode.UNKNOWN_ERROR;
		}
